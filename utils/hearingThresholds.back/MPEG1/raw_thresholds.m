...
% is 384 the fft shift
for OFFSET = 1:384:length(x)-384
    % Compute the FFT for time frequency conversion [1, pp. 110].
    X = [X, FFT_Analysis(x, OFFSET)];
end
%size(X) = (512,185), 512 is the fft size, 185 is number of frames
max_val = max(max(X));
%-----so basically above steps just calculate the maximum value of fft
%transormation
