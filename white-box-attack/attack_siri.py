...
#        print(model.layers)
    ...
    def attack_simple(self, source_wav, target_speaker, path_to_save, target_threshold):
        f = open(self.log_file, 'a+')
        success = 0
        source_wav = self.dataset_dir + '/' + source_wav.split('_')[0] + '/' + source_wav

        #0. load target speaker embedding
        target_embedding_vector = np.load(self.embedding_dir + target_speaker + '.npy')
        ...
        original_audio = original_audio[:, :25840]
...
if __name__ == '__main__':
    ...
    output_dir = '../data/adversarial_siri/'
    log_file = './output_threshold_siri.log'
    attack  = Attack(checkpoint_path, step_size = 0.01, num_steps = 400, wav_dir = '../data/siri/', 
                output_dir = output_dir, embedding_dir = '../data/embeddings_siri/', log_file = log_file)
    meta_file = './trials_siri.txt'
    ...
    for line in f.readlines():
        ...
        #target_phrase = '37215684'
        #target_speaker = arr[1]
        adv_path = output_dir + source_wav.split('/')[-1] + '_' + target_speaker  + '.wav'
        
        #threshold = attack.find_threshold_for_target(target_speaker)
        threshold = 0.75
        print(source_wav, target_speaker, adv_path, threshold)
        attack.attack_simple(source_wav, target_speaker, adv_path, target_threshold = threshold)
