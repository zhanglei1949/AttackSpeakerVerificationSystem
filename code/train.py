...
def main(vox_dir=c.DATASET_DIR):

    PRE_TRAIN = c.PRE_TRAIN
    logging.info('Looking for fbank features [.npy] files in {}.'.format(vox_dir))
    vox_data = load_dataset(vox_dir)
    
    unique_speakers = vox_data['speaker_id'].unique()
    spk_utt_dict, unique_speakers = create_dict(vox_data['filename'].values, vox_data['speaker_id'].values, unique_speakers)
    logging.info('obtaining totally {} unique speakers with at least 1 utterance.'.format(len(unique_speakers)))
    select_batch.create_data_producer(unique_speakers, spk_utt_dict)

    batch = stochastic_mini_batch(vox_data, batch_size=c.BATCH_SIZE, unique_speakers=unique_speakers)
    batch_size = c.BATCH_SIZE * c.TRIPLET_PER_BATCH
    # Actual batch size is batchsize * 3
    x, y = batch.to_inputs()
    b = x[0]
    ...
    while True:
        ...
        # This is actually not used in loss calculation
        ...
        if (grad_steps ) % c.TEST_PER_EPOCHS == 0 :
            fm, tpr, acc, eer = eval_model(model,train_batch_size, test_dir = c.TEST_DIR, check_partial = False, gru_model=gru_model)
